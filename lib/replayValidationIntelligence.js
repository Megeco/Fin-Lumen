...
} from "./finLumenCapitalLanguage.js";
...
function receptorFitCapitalGate(transitReceptorFit, capitalHorizon) {
  ...
  const near = transitReceptorFit.components?.pressureInterference?.nearFieldGate || null;
  const next = JSON.parse(JSON.stringify(capitalHorizon || {}));
  const blockFresh = TRM_BLOCK_FRESH_CLASSES.has(cls);
  const muted = TRM_MUTED_CLASSES.has(cls) || cls === "SUPPORTIVE_BUT_MUTED";
  const contested = TRM_CONTESTED_CLASSES.has(cls);
  const reading = trmReading(transitReceptorFit);
  const label = transitReceptorFit.expressionLabel || cls.replaceAll("_", " ");

  if (blockFresh) {
    const pressureBlock = cls.includes("PRESSURE") || cls.includes("ROTATION");
    next.tactical = {
      ...(next.tactical || {}),
      verdict: pressureBlock ? "NO FRESH ENTRY — PROTECT / OBSERVE" : "SELECTIVE FRESH CAPITAL ONLY",
      displayVerdict: pressureBlock
        ? "Pressure-first: fresh capital waits; protect/observe until pressure clears."
        : "Fresh capital: selective / staggered only; do not chase the window.",
      note: `${label}: ${reading || "Transit receptor fit is weak or pressure-led."}`
    };
    next.strategic = {
      ...(next.strategic || {}),
      verdict: pressureBlock ? "AVOID FRESH STRATEGIC CAPITAL" : "SELECTIVE STRATEGIC PARTICIPATION",
      displayVerdict: pressureBlock
        ? "Fresh strategic allocation waits until pressure clears."
        : "Strategic: participate selectively; the window is active, but sizing remains capped.",
      note: pressureBlock
        ? "Macro weather does not convert into investable company-specific expression through sector/receptor/pressure checks."
        : "Macro weather is not a clean standalone driver; stock-specific natal timing still controls sizing."
    };
    next.passiveLongTerm = {
      ...(next.passiveLongTerm || {}),
      verdict: pressureBlock ? "NO — NOT PASSIVE LONG-TERM" : "ACTIVE REVIEW REQUIRED",
      displayVerdict: pressureBlock
        ? "Not an autopilot/passive allocation at this window."
        : "Not an autopilot/passive add; use active review windows and selective sizing.",
      note: pressureBlock
        ? "Company-specific transit expression is pressure/non-responsive until a new receptor window appears."
        : "Company-specific expression is selective until the next stronger receptor window appears."
    };
  } else if (muted) {
    next.tactical = {
      ...(next.tactical || {}),
      verdict: "HOLD / WAIT — SUPPORTIVE BUT MUTED",
      displayVerdict: "Hold or wait; supportive but not leadership-grade.",
      note: `${label}: sector weather exists, but natal receptor or support-network strength is not enough for leadership language.`
    };
    if (String(next.strategic?.verdict || "").includes("YES")) {
      next.strategic.verdict = "CONDITIONAL — COMPANY-SPECIFIC TRIGGER REQUIRED";
      next.strategic.displayVerdict = "Conditional allocation; company-specific trigger required.";
    }
  } else if (contested) {
    next.tactical = {
      ...(next.tactical || {}),
      verdict: near?.severity && near.severity !== "CLEAR" ? "HOLD / PROTECT FIRST" : "HOLD / PART-SIZED ONLY",
      displayVerdict: near?.severity && near.severity !== "CLEAR" ? "Hold only with capital-protection discipline." : "Selective participation only.",
      note: `${label}: support exists but pressure interference prevents clean rerating language.`
    };
    if (near?.severity && near.severity !== "CLEAR") {
      next.breakHorizon = next.breakHorizon || {
        date: near.date,
        days: near.days,
        pressure: near.severity,
        label: near.severity,
        evidence: [near.reason],
        phase: "NEAR-FIELD PROTECTION"
      };
    }
  }

  ["tactical", "strategic", "passiveLongTerm"].forEach(key => {
    if (next[key]) next[key].displayVerdict = next[key].displayVerdict || softenCapitalVerdict(next[key].verdict);
  });

  next.transitReceptorGate = {
    expressionClass: cls,
    expressionLabel: transitReceptorFit.expressionLabel,
    expressionScore: trmExpressionScore(transitReceptorFit, null),
    confidenceScore: transitReceptorFit.scores?.confidenceScore ?? null,
    reading,
    nearFieldGate: near,
    rule: "TRM can downgrade capital language when macro weather does not pass sector/receptor/pressure/fundamental gates."
  };
  return next;
}
...
function tacticalStateAfterReceptorFit(baseTactical, transitReceptorFit) {
  ...
  const cls = String(transitReceptorFit.expressionClass || "");
  if (TRM_BLOCK_FRESH_CLASSES.has(cls)) return "NO FRESH ENTRY — PROTECT / OBSERVE";
  if (TRM_MUTED_CLASSES.has(cls) || cls === "SUPPORTIVE_BUT_MUTED") return "HOLD / WAIT — SUPPORTIVE BUT MUTED";
  if (TRM_CONTESTED_CLASSES.has(cls)) return transitReceptorFit.components?.pressureInterference?.nearFieldGate?.severity !== "CLEAR" ? "HOLD / PROTECT FIRST" : "HOLD / PART-SIZED ONLY";
  return baseTactical;
}
...
export function buildReplayValidationIntelligence({ replayDate, replay, windows, macroSnapshot, natalResearch, forwardDays = 730, transitReceptorFit = null }) {
  ...
  let capitalHorizon = receptorFitCapitalGate(transitReceptorFit, baseCapitalHorizon);
  const baseDisagreements = modelDisagreements(present, timeline, windows);
  const contradictionReport = buildContradictionReport({
    replay: present,
    windows,
    transitReceptorFit,
    capitalHorizon,
    currentDormancy,
    strategic,
    modelDisagreements: baseDisagreements
  });
  const confidenceCalibration = calibrateInterpretationConfidence({
    transitReceptorFit,
    contradictionReport,
    replayValidationIntelligence: { disagreements: baseDisagreements },
    natalResearch
  });
  if (["HIGH", "CRITICAL"].includes(contradictionReport.severity)) {
    if (capitalHorizon?.strategic && /YES|BUILD|CORE|ACCUMULATE/.test(String(capitalHorizon.strategic.verdict || "").toUpperCase())) {
      capitalHorizon.strategic.verdict = "CONDITIONAL — CONTRADICTION REVIEW REQUIRED";
      capitalHorizon.strategic.displayVerdict = "Conditional allocation; contradiction review required.";
      capitalHorizon.strategic.note = `${capitalHorizon.strategic.note || ""} Contradiction Engine caps strategic language.`;
    }
    if (capitalHorizon?.passiveLongTerm && /YES|LONGER/.test(String(capitalHorizon.passiveLongTerm.verdict || "").toUpperCase())) {
      capitalHorizon.passiveLongTerm.verdict = "CONDITIONAL — CONFIDENCE CAPPED";
      capitalHorizon.passiveLongTerm.displayVerdict = "Longer runway conditional; confidence capped.";
      capitalHorizon.passiveLongTerm.note = `${capitalHorizon.passiveLongTerm.note || ""} Confidence calibration requires evidence before passive approval.`;
    }
  }
  ...
  return {
    ...
    version: "v33.1-synthesis-hardening",
    chartContext: {
    ...
    disagreements: baseDisagreements,
    contradictionReport,
    confidenceCalibration,
    chronologicalStory: `${coherentStory(timeline, currentDormancy, futureDormancy, strategic)} Capital horizon: Tactical ${capitalHorizon.tactical.verdict}; Strategic ${capitalHorizon.strategic.verdict}; Passive long-term ${capitalHorizon.passiveLongTerm.verdict}. Confidence ${confidenceCalibration.label} (${confidenceCalibration.score}/100). Contradiction severity ${contradictionReport.severity}.`,
