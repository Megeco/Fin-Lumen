...
export function softenCapitalVerdict(verdict = "") {
  ...
  if (upper.includes("NO FRESH ENTRY")) return "Pressure-first: fresh capital waits; protect/observe until pressure clears.";
  if (upper.includes("AVOID FRESH STRATEGIC CAPITAL")) return "Fresh strategic allocation waits until pressure clears.";
  if (upper.includes("SELECTIVE FRESH CAPITAL ONLY")) return "Fresh capital: selective / staggered only; do not chase the window.";
  if (upper.includes("SELECTIVE STRATEGIC PARTICIPATION")) return "Strategic: participate selectively; the window is active, but sizing remains capped.";
  if (upper.includes("ACTIVE REVIEW REQUIRED")) return "Not an autopilot/passive add; use active review windows and selective sizing.";
  if (upper.includes("BLOCK FRESH")) return "Fresh allocation blocked by receptor/cycle risk.";
  if (upper.includes("NO — NOT PASSIVE")) return "Not an autopilot/passive allocation at this window.";
